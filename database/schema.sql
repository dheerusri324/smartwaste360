-- Smart Waste 360 - Complete Database Schema
-- PostgreSQL compatible

-- Create database
CREATE DATABASE smartwaste360;
...
CREATE TABLE collectors (
    ...
    phone VARCHAR(15),
    email VARCHAR(100),
    assigned_colonies TEXT, -- JSON array of colony_ids
...
CREATE TABLE user_transactions (
    ...
    points_earned INT,
    materials JSONB, -- {"plastic": 2.5, "paper": 1.0, ...}
...
BEGIN
    IF TG_OP = 'INSERT' THEN
        UPDATE colonies 
        SET total_points = colonies.total_points + NEW.points_earned
        FROM users
        WHERE colonies.colony_id = users.colony_id AND users.user_id = NEW.user_id;
    ELSIF TG_OP = 'UPDATE' THEN
        UPDATE colonies 
        SET total_points = colonies.total_points + (NEW.points_earned - OLD.points_earned)
        FROM users
        WHERE colonies.colony_id = users.colony_id AND users.user_id = NEW.user_id;
    END IF;
    RETURN NEW;
